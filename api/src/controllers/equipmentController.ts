...
export const getEquipmentTypes = async (req: Request, res: Response) => {
  try {
    ...
    let whereConditions: string[] = ['et.company_id = $1'];
...
export const createEquipmentType = async (req: Request, res: Response) => {
  try {
    ...
    // Check if equipment type already exists for this company
    const existingQuery = `
      SELECT id FROM equipment_types
      WHERE brand = $1 AND model = $2 AND company_id = $3
    `;
    const existingResult = await query(existingQuery, [brand, model, companyId]);
    ...
    const insertQuery = `
      INSERT INTO equipment_types (
        company_id, name, brand, model, description, manual_url, warranty_period_months, image_url
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *
    `;

    const result = await query(insertQuery, [
      companyId, name, brand, model, description, manual_url, warranty_period_months, image_url
    ]);
...
export const getCustomerEquipment = async (req: Request, res: Response) => {
  try {
    ...
    const companyId = req.company?.id || '9703f477-46e1-47d0-a0f4-fa1eeb521499';
...
export const createCustomerEquipment = async (req: Request, res: Response) => {
  try {
    console.log('Creating customer equipment with body:', JSON.stringify(req.body, null, 2));
    ...
    console.log('Validation check:', { customer_id, equipment_type_id, serial_number });
    ...
    console.log('Checking for existing serial number:', serial_number, 'in company:', companyId);
    const existingQuery = `
    ...
    console.log('Existing equipment check result:', existingResult.rows.length);

    if (existingResult.rows.length > 0) {
      console.log('ERROR: Serial number already exists');
      return res.status(400).json({
        success: false,
    ...
    console.log('Checking customer:', customer_id, 'in company:', companyId);
    ...
    console.log('Customer check result:', customerResult.rows.length);
    ...
    console.log('Checking equipment type:', equipment_type_id);
    ...
    console.log('Equipment type check result:', equipmentTypeResult.rows.length);

    if (equipmentTypeResult.rows.length === 0) {
      console.log('ERROR: Equipment type not found');
      return res.status(400).json({
        success: false,
        error: 'Equipment type not found'
      } as ApiResponse);
