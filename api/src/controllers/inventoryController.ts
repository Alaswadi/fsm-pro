...
export const createInventoryItem = async (req: Request, res: Response) => {
  try {
    ...
    const insertQuery = `
      INSERT INTO parts (
        company_id, part_number, name, description, category, brand,
    ...
    const result = await query(insertQuery, [
      companyId, part_number, name, description, category, brand,
      unit_price, cost_price, current_stock, min_stock_level, max_stock_level,
      status, supplier_info, compatible_equipment, image_url
    ]);
...
// Get inventory options for forms (categories, brands, etc.)
export const getInventoryOptions = async (req: Request, res: Response) => {
  try {
    ...
    // Get unique brands from equipment_types (this ensures brand consistency)
    const brandsResult = await query(
      'SELECT DISTINCT brand FROM equipment_types WHERE company_id = $1 AND brand IS NOT NULL ORDER BY brand',
      [companyId]
    );
