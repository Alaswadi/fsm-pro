...
export const updateCustomer = async (req: Request, res: Response) => {
  try {
    ...
    // Build update query - only update is_active if it's explicitly provided
    let updateQuery: string;
    let queryParams: any[];

    if (is_active !== undefined) {
      // If is_active is provided, include it in the update
      updateQuery = `
      ...
      // If is_active is not provided, don't update it (preserve current value)
      updateQuery = `
        UPDATE customers SET
          name = $1,
          email = $2,
      ...
      queryParams = [
        ...
        preferred_contact_method, service_tier, contract_type, cleanContractStartDate,
        cleanContractEndDate, payment_terms, credit_limit, discount_percentage,
        priority_level, cleanAssignedAccountManager, notes, id, companyId
      ];
    }
